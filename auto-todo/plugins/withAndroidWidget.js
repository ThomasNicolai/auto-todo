...
function addImportToKotlinFile(content, importStatement) {
...
function copyWidgetFiles(projectRoot, platformRoot, packageName) {
    const packagePath = packageName.split(".").join(path.sep);
    const srcDir = path.join(platformRoot, "app", "src", "main", "java", packagePath);
    ...
    const resLayoutDir = path.join(projectRoot, "plugins", "android-widget", "res", "layout");
    ...
    fs.copyFileSync(path.join(projectRoot, "plugins", "android-widget", "res", "xml", "todo_widget_info.xml"), path.join(xmlDir, "todo_widget_info.xml"));
}
function patchMainApplication(platformRoot, packageName) {
    const packagePath = packageName.split(".").join(path.sep);
    const mainAppPath = path.join(platformRoot, "app", "src", "main", "java", packagePath, "MainApplication.kt");
    if (!fs.existsSync(mainAppPath))
        return;
    let content = fs.readFileSync(mainAppPath, "utf8");
    if (content.includes("TodoWidgetBridgePackage"))
        return;
    content = addImportToKotlinFile(content, `import ${packageName}.TodoWidgetBridgePackage`);
    ...
    if (content.includes(primaryAnchor)) {
        content = content.replace(primaryAnchor, `${primaryAnchor}\n          add(TodoWidgetBridgePackage())`);
    }
...
function addWidgetToManifest(manifest, packageName) {
    const app = config_plugins_1.AndroidConfig.Manifest.getMainApplication(manifest);
    if (!app)
        return;
    if (!app.receiver)
        app.receiver = [];
    const alreadyHasWidget = app.receiver.some((r) => r.$?.["android:name"] === ".TodoWidgetProvider");
    if (!alreadyHasWidget) {
        app.receiver.push({
            $: {
    ...
    if (!app.service)
        app.service = [];
    const alreadyHasService = app.service.some((s) => s.$?.["android:name"] === ".TodoWidgetService");
...
const withAndroidWidget = (config) => {
    config = (0, config_plugins_1.withDangerousMod)(config, [
        ...
        (config) => {
            ...
            if (!packageName) {
                throw new Error("[withAndroidWidget] android.package must be set in app.json before running expo prebuild.");
            }
...
exports.default = withAndroidWidget;
